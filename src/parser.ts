...
export function generate() {
  ...
  cleanOldGeneratedFiles(OUTPUT_DIR);
...
export function generateModel(model: PrismaModel, enums: PrismaEnum[]): string {
  ...
  // Built-in JS types that must never be imported as model references
  const builtIns = new Set(['String', 'Number', 'Boolean', 'Date', 'Object', 'Array', 'Set', 'Map']);
  ...
  // Filter out self-referential imports — the class is declared in this file
  referencedModels.delete(model.name);
  ...
  for (const ref of [...referencedEnums].sort()) {
    code += `import { ${ref}, ${ref}Values } from './${ref}';\n`;
  }
  ...
  code += `  static override get schema(): SchemaDefinition {\n`;
  code += `    const s = ${model.name}._schema;\n`;
  code += `    if (s) {\n`;
  code += `      return s;\n`;
  code += `    }\n`;
  code += `    const newSchema = {\n`;

  code += fieldEntries.map((fe) => fe.entry).join('\n');
  code += `\n      } as const satisfies SchemaDefinition;\n`;
  code += `    ${model.name}._schema = newSchema;\n`;
  code += `    return newSchema;\n`;
  code += `  }\n`;
  code += `}\n`;
...
function generateFieldEntry(field: PrismaField, enumNames: Set<string>): { entry: string; usedTypes: string[] } | null {
  ...
  if (field.isArray) {
    ...
    if (isEnumType) valParts.push(`enum: ${field.type}Values`);
    if (isModelType) valParts.push('coerce: true');
    config.push(`values: { ${valParts.join(', ')} }`);
    if (!isEnumType) usedTypes.push(mcType);
    if (isEnumType) usedTypes.push(field.type);
  }

  // Enum field type — uses the exported Values array, not the TS enum object
  if (isEnumType && !field.isArray) {
    config.push(`enum: ${field.type}Values`);
